...
class PingWeek extends Command
{
    /**
     ...
     * @var string
     */
    protected $signature = 'app:ping-week';
    ...
    public function handle(BotService $botService)
    {
        ...
        $users = User::query()
            ...
            ->whereBetween('reminders.remind_week', [$start, $end])
            ->select('users.*')
            ->distinct()
            ->get();
        foreach ($users as $user){
            $botService->sendMessageToUser( $user, __('messages.reminders.week'));
